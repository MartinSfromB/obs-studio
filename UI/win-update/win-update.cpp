...
try {
	...
	if (blake2b_init (&blake2, BLAKE2_HASH_LENGTH))
		return false;
	...
	for (;;) {
		...
		if (blake2b_update(&blake2, buf.data(), read))
			return false;
	}

	if (blake2b_final(&blake2, hash, BLAKE2_HASH_LENGTH))
		return false;
...
try {
	...
	if (CalculateFileHash(updateFilePath, updateFileHash)) {
		char hashString[(BLAKE2_HASH_LENGTH * 2) + 1];
